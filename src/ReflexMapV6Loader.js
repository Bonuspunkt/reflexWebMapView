...
ReflexMapV6Loader.prototype = {
  ...
  parse: function ( text, callbackFinished, url ) {
    ...
    result = {
      ...
      objects: {},
      cameras: [
        new THREE.PerspectiveCamera( 90, window.innerWidth / window.innerHeight, 1, 10000 )
      ],
      lights: {},
      fogs: {},
      empties: {},
    ...
    map.entities.forEach(function(entity) {
      ...
      if (type === 'playerSpawn') {
        ...
        if (entity.angles) {
          ...
          Object.keys(spawn.rotation).forEach(function(key) {
            spawn.rotation *= Math.PI / 180;
          });
        }
        result.cameras.push(spawn);
      }

      if (type === 'target' && entity.name === gameOverCamera) {
        var spawn = { position: entity.position };
        if (entity.angles) {
          spawn.rotation = entity.angles;
          Object.keys(spawn.rotation).forEach(function(key) {
        ...
        result.cameras.unshift(camera);
      }


      if (type === 'worldSpawn') {
        gameOverCamera = entity.targetGameOverCamera;
        // result.scene.add(new THREE.HemisphereLight(entity.clearColor, entity.worldColor1));
      }
      ...
      entity.brushes.forEach(function(brush) {
        ...
        if (!result.groups[type]) { result.groups[type] = []; }
        result.groups[type].push(mesh);
